# <img src="public/images/logo.svg" width="32" height="32" align="center"> EdgePulse - EdgeOne Makers 原生在线监控与状态页
...
- ⚙️ **前端可视化管理 Modal**：支持在 UI 界面点击【⚙️ 管理监控】弹窗，快速添加监控节点。
  ...
  - 🧪 **常驻本地测试数据生成与隔离清理 (Local Storage Only)**：支持已登录管理员一键生成 5 种监控类型各 3 个（共 15 个）不重样的测试数据。测试节点严格限定仅保存在当前浏览器的 LocalStorage 中，**绝不上传与污染线上 KV 数据库**，并提供**独立无损的【清理模拟测试数据】功能**。
- 🔔 **丰富多渠道告警通知**：
